...
# 抑制 SSL 不安全警告（由 GITHUB_VERIFY_SSL=false 引起）
...
def _get_repo():
    ...
    if _github_client is None:
        _github_client = Github(
            ...
            verify=settings.github_verify_ssl,
        )
    ...
    try:
        ...
        for label in labels:
            if label not in existing:
                try:
                    repo.create_label(name=label, color="3B82F6")
                    import warnings as _w
                    _w.warn(f"自动创建标签: {label}")
...
def create_issue(
    ...
    try:
        # 确保标签存在
        _ensure_labels_exist(labels or [])
        repo = _get_repo()
